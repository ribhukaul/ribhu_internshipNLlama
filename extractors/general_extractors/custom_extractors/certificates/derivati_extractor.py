...
from extractors.utils import upload_df_as_excel
...
class DerivatiKidExtractor(Extractor):
    ...
    async def extract_callable(self):
        ...
        try:
            for key in is_callable.keys():
                pattern = re.compile(regex_callable[key], re.IGNORECASE)
                is_callable[key] = bool(
                    pattern.search(getattr(self.text[0], "page_content", "ERROR"))
                )

            if is_callable["softcallable"]:
                is_callable["autocallable"] = 0

        except Exception as error:
            print("extract_callable error" + repr(error))
            is_callable = {key: "ERROR" for key, value in is_callable.items()}
        return is_callable
